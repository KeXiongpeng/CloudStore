...
- [x] 实现成员列表。
- [x] 实现邮箱邀请、接受邀请、角色修改、移除成员。
- [x] 邀请令牌只保存 SHA-256 哈希。
- [x] 邀请默认 7 天过期。
- [x] 禁止修改或移除 `OWNER`。
- [x] 所有敏感变更写审计日志。
- [x] 提交：`feat(workspace): add invitations and member management`
...
- [x] 文件列表、详情、删除改为工作区作用域。
- [x] 公开预览继续支持旧 `urlKey`。
- [x] 公开文件仍要求工作区 active 且文件未删除。
- [x] 更新架构、数据库、API 文档。
- [x] 跑完服务端单测、e2e、客户端 lint、客户端构建。
- [x] 提交：`feat(workspace): complete rbac regression and docs`

### 任务 6.6：修复刷新令牌错误并补齐审计页面

- [x] `refresh token` 过期/无效不再返回 `500`，统一返回 `401`。
- [x] 过期提示为「登录已过期，请重新登录」。
- [x] 增加刷新令牌异常单元测试。
- [x] 工作区创建、更新、删除写入审计日志。
- [x] 新增工作区审计日志页面 `/workspaces/:workspaceId/audit`。
- [x] 工作区切换器支持复制当前工作区 ID。
- [x] 增加审计日志 e2e 测试。
- [x] 服务端测试、类型检查、构建通过。
- [x] 客户端 lint、构建通过。
- [x] 代码依赖图无违规。
