...
一个使用 Next.js、NestJS、PostgreSQL、Redis 和 S3 兼容对象存储构建的云存储与文件分享平台。支持文件直传、大文件分片上传、在线预览、公开分享链接、二维码、访问统计、用户配额和管理后台。

> 这是一个面向面试展示的全栈学习项目，重点覆盖前后端协作、对象存储、认证授权、容器化部署和响应式 Web UI。
